...
describe('Message Handlers', () => {
	describe('MessageHandler type', () => {
		it('should accept async function that returns TextMessage', async () => {
			...
			const mockAnnotater: Annotater = { annotateToolCall: async () => {} };

			const result = await handler('session-data', input, mockAnnotater);
	...
	describe('PresenceHandler type', () => {
		it('should accept void function', () => {
			const handler = (_data: string, msg: PresenceMessage): void => {
				expect(msg.sender).toBeDefined();
			};

			const presence = new PresenceMessage(PresenceStatus.Available, 'alice');
			handler('session-data', presence);
		});

		it('should accept async void function', async () => {
			const handler = async (_data: string, msg: PresenceMessage): Promise<void> => {
			...
			await handler('session-data', presence);
		});

		it('should allow side effects in handler', async () => {
			const log: string[] = [];

			const handler = (data: string, msg: PresenceMessage): void => {
				log.push(`${data}: ${msg.status}`);
			};
